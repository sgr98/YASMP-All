...
const saveUserFile = async (obj, path, type) => {
    ...
    try {
        ...
        console.error('ERROR WRITING (' + type + '): Error writing file\n', error);
    }
};
...
const readUserFile = async (path, type) => {
    ...
    try {
        ...
        console.error('ERROR WRITING (' + type + '): Error reading file\n', err);
    }
    return userData;
};
...
const createContactList = (contactList, existingContactList) => {
    if (existingContactList === undefined)
        existingContactList = []
...
const createUserSpace = (user, contactList, existingUserSpace) => {
    ...
    userSpace["contacts"] = contactList
...
export const createDir = async (dirPath) => {
    try {
        ...
        if (err.code === "EEXIST")
            ...
            console.error("ERROR: Dir does not exists and encountered error creating it")
        else
            console.error("ERROR: Error creating dir\n", err)
    }
}
